...
def make_console(progress_enabled: bool = True):
    from rich.console import Console

    return Console(force_terminal=progress_enabled)
...
def storage_uri_to_path(uri: Optional[str]) -> Optional[Path]:
    if not uri:
        return None
    parsed = urlparse(uri)
...
def find_payload_path(store: "Store", fmt: str, sha256: str) -> Optional[Path]:
    base = store.root / fmt
    if not base.exists():
        return None
    matches = list(base.glob(f"{sha256}.*"))
...
class StatsSnapshot:  # pylint: disable=too-many-instance-attributes
    total: int
    completed: int
    ok: int
    failed: int
    ...
    other_errors: int
    bytes: int
    ok_by_fmt: Dict[str, int]
    downloaded_by_fmt: Dict[str, int]


@dataclass
class StatsTracker:  # pylint: disable=too-many-instance-attributes
    total: int = 0
    completed: int = 0
    ok: int = 0
    failed: int = 0
    ...
    other_errors: int = 0
    bytes: int = 0
    ok_by_fmt: Dict[str, int] = field(
        default_factory=lambda: {"xml": 0, "json": 0, "pdf": 0}
    )
    downloaded_by_fmt: Dict[str, int] = field(
        default_factory=lambda: {"xml": 0, "json": 0, "pdf": 0}
    )
    ...
    async def record_result(
        ...
        async with self._lock:
            ...
            if ok:
                self.ok += 1
                if fmt in self.ok_by_fmt:
                    self.ok_by_fmt[fmt] += 1
                if bytes_read:
                    self.bytes += bytes_read
                    if fmt in self.downloaded_by_fmt:
                        self.downloaded_by_fmt[fmt] += 1
                if status == 304:
                    self.not_modified += 1
                return
    ...
    async def snapshot(self) -> StatsSnapshot:
        async with self._lock:
            return StatsSnapshot(
                ...
                bytes=self.bytes,
                ok_by_fmt=dict(self.ok_by_fmt),
                downloaded_by_fmt=dict(self.downloaded_by_fmt),
            )
...
def make_status_panel(
    ...
    total_text = str(snapshot.total) if snapshot.total else "-"
    bytes_text = format_bytes(snapshot.bytes)

    table = Table(box=box.SIMPLE, pad_edge=False, show_header=False)
    table.add_column("Metrica", style="bold")
    table.add_column("Valor", justify="right")

    table.add_row("Items completados", f"{snapshot.completed} / {total_text}")
    table.add_row("Descargas OK", str(snapshot.ok))
    table.add_row("Descargas KO", str(snapshot.failed))
    table.add_row("No modificados (304)", str(snapshot.not_modified))
    table.add_row("HTTP 3xx", str(snapshot.http_3xx))
    table.add_row("HTTP 4xx", str(snapshot.http_4xx))
    table.add_row("HTTP 5xx", str(snapshot.http_5xx))
    table.add_row("Timeouts", str(snapshot.timeouts))
    table.add_row("Errores cliente", str(snapshot.client_errors))
    table.add_row("Otros errores", str(snapshot.other_errors))
    table.add_row("Bytes descargados", bytes_text)
    table.add_row("XML OK", str(snapshot.ok_by_fmt.get("xml", 0)))
    table.add_row("JSON OK", str(snapshot.ok_by_fmt.get("json", 0)))
    table.add_row("PDF OK", str(snapshot.ok_by_fmt.get("pdf", 0)))
    table.add_row("XML descargados", str(snapshot.downloaded_by_fmt.get("xml", 0)))
    table.add_row("PDF descargados", str(snapshot.downloaded_by_fmt.get("pdf", 0)))
    table.add_row("Concurrentes", str(concurrency))
    table.add_row("CPU", cpu_pct)
    table.add_row("RAM", rss_text)

    return Panel(table, title="Estado de descarga", border_style="cyan")
...
DB_GET_RESOURCE_FORMAT = """
SELECT
  {fmt}_downloaded AS downloaded,
  {fmt}_sha256 AS sha256,
  {fmt}_storage_uri AS storage_uri
...
async def download_one_format(
    *,
    ...
    if db and resource_id != "NO_DB":
        downloaded, sha_existing, storage_uri_existing = (
            await db.get_resource_format_status(resource_id, fmt)
        )
        if downloaded:
            payload_path = storage_uri_to_path(storage_uri_existing)
            if payload_path is None and sha_existing:
                payload_path = find_payload_path(store, fmt, sha_existing)
                if payload_path:
                    storage_uri_existing = f"file://{payload_path.resolve()}"
            if payload_path and payload_path.exists():
                if stats:
                    await stats.record_result(fmt, True, 200, 0, None)
                return True, sha_existing, storage_uri_existing, None
    ...
    for attempt in range(retries + 1):
        ...
        try:
            ...
            async with session.get(
                ...
                if status == 304:
                    ...
                    if stats:
                        await stats.record_result(fmt, True, 304, 0, None)
                            ...
                            if (
                                ...
                                if stats:
                                    await stats.record_result(
                                        fmt, True, status, bytes_read, None
                                    )
        ...
        if stats:
            await stats.record_result(fmt, False, status, None, error_type)
        return False, None, None, content_type

    if stats:
        await stats.record_result(fmt, False, None, None, "unknown")
...
async def run_sumario(
    ...
    if db:
        ...
        with Live(
            Spinner("dots", text="Estimando tiempo..."),
            console=console,
            refresh_per_second=8,
            transient=True,
        ):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if "json" in formats:
            specs.append(("json", url, "application/json"))
        if "xml" in formats:
            specs.append(("xml", url, "application/xml"))
        if "pdf" in formats and url_pdf:
            specs.append(("pdf", url_pdf, "application/pdf"))
        ...
        prog = make_progress(console)
        stats = StatsTracker()
        await stats.add_total(len(specs))
        task_id = prog.add_task("download", total=len(specs))

        proc: Optional[Any] = (
            psutil.Process() if psutil is not None else None  # type: ignore
        )
        if proc is not None:
            try:
        ...
        async def ui_loop(live) -> None:
            while not stop_event.is_set():
                snap = await stats.snapshot()
                cpu_val = proc.cpu_percent(interval=None) if proc else None
                cpu_pct = f"{cpu_val:.1f}%" if cpu_val is not None else "n/a"
                rss_val = (proc.memory_info().rss / 1024 / 1024) if proc else None
                mem_pct_val = (
                    psutil.virtual_memory().percent if psutil is not None else None
                )
                if rss_val is not None and mem_pct_val is not None:
                    rss_text = f"{rss_val:.1f} MB ({mem_pct_val:.1f}%)"
                elif rss_val is not None:
                    rss_text = f"{rss_val:.1f} MB"
                elif mem_pct_val is not None:
                    rss_text = f"{mem_pct_val:.1f}%"
                else:
                    rss_text = "n/a"
                panel = make_status_panel(snap, args.concurrency, cpu_pct, rss_text)
                live.update(Group(prog, panel))
                prog.refresh()
        ...
        panel = make_status_panel(
            await stats.snapshot(), args.concurrency, "n/a", "n/a"
        )
        with Live(
            Group(prog, panel),
            console=console,
            ...
            try:
                tasks = [
                    asyncio.create_task(guarded(fmt, u, accept))
                    for (fmt, u, accept) in specs
                ]
...
async def run_consolidada(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async def process_one(boe_id: str) -> None:
            ...
            if total_add:
                await stats.add_total(total_add)
        ...
        prog = make_progress(console)
        task_id = prog.add_task("download", total=len(boe_ids))
        ...
        async def ui_loop(live) -> None:
            while not stop_event.is_set():
                ...
                if rss_val is not None and mem_pct_val is not None:
                    ...
                    rss_text = "n/a"
                panel = make_status_panel(snap, workers, cpu_pct, rss_text)
                live.update(Group(prog, panel))
                prog.refresh()
                await asyncio.sleep(ui_refresh)
        ...
        panel = make_status_panel(await stats.snapshot(), workers, "n/a", "n/a")
        with Live(
            Group(prog, panel),
            console=console,
            refresh_per_second=8,
